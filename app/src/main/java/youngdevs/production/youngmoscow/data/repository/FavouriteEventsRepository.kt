...
class FavouriteEventsRepository(private val favouriteEventsDao: FavouriteEventsDao) {
    ...
    // Удаление события из списка избранных по его идентификатору с помощью метода removeFromFavourites()
    suspend fun removeFromFavourites(eventId: Int) =
        favouriteEventsDao.removeFromFavourites(eventId)

    // Получение события из таблицы EventFavourite по его идентификатору с помощью метода getEventById()
